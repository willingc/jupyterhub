...
Cookies are sent to the Hub for verification, replying with a JSON model
...
class _ExpiringDict(dict):
    ...
    def _check_age(self, key):
        ...
        if key not in self.values:
            # key not registered, nothing to do
            return
        now = time.monotonic()
...
class HubAuth(Configurable):
    ...
    # hub location
    api_url = Unicode(os.environ.get('JUPYTERHUB_API_URL') or
    ...
    api_token = Unicode(os.environ.get('JUPYTERHUB_API_TOKEN', ''),
                        help="""API key for accessing Hub API.
                             ...
                             Generate with `jupyterhub token [username]` or
                             add to JupyterHub.services config.
                             """
                       ).tag(config=True)

    cookie_name = Unicode('jupyterhub-services',
                          help="""The name of the cookie I should be using"""
                         ).tag(config=True)

    cookie_cache_max_age = Integer(300,
                                   help="""The maximum time (in seconds) to
                                        cache the Hub's response for cookie
                                        authentication.

                                        A larger value reduces load on the Hub
                                        and occasional response lag.
                                        A smaller value reduces propagation
                                        time of changes on the Hub (rare).
    ...
    def user_for_cookie(self, encrypted_cookie, use_cache=True):
        ...
        try:
            ...
            if '127.0.0.1' in self.api_url:
                msg += ("  Make sure to set c.JupyterHub.hub_ip to an IP"
                        "accessible to single-user servers if the servers are"
        ...
        if r.status_code == 404:
            ...
            app_log.error("I don't have permission to verify cookies, my auth"
                          "token may have expired: [%i] %s",
                          r.status_code, r.reason)
            raise HTTPError(500, "Permission failure checking authorization,"
                                 "I may need a new token")
        elif r.status_code >= 500:
            app_log.error("Upstream failure verifying auth token: [%i] %s",
                          r.status_code, r.reason)
            raise HTTPError(502, "Failed to check authorization (upstream "
                                 "problem)")
...
class HubAuthenticated(object):
    ...
    A handler using this mixin must have the following attributes/properties:
